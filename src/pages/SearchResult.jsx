    ...
    const fetchSearchResults = async () => {
      if (query) {
        ...
        for (const category of categories) {
          ...
          const snapshot = await ref.orderByChild(category.index).startAt(query).endAt(query + "\uf8ff").once("value");

          const data = snapshot.val();
          if (data) {
            const categoryResults = Object.entries(data).map(([key, value]) => ({ key, ...value, category: category.name }));
            results = [...results, ...categoryResults];
          }
        }
