...
/* doesnt work btn search but enter correct work */
...
/* eslint-disable react/prop-types */
/* doesnt work blur keyboard but btn search, enter correct work */
...
export default function InputSearch({
  ...
  const [isInputFocused, setInputFocused] = useState(false);
  const [inputValue, setInputValue] = useState(searchTerm || "");
  const inputRef = useRef(null); // Ref do elementu input
  const navigate = useNavigate();
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      handleSearch();
      inputRef.current.blur(); // Ukryj klawiaturę
    }
  };
  ...
  return (
    <div className={`search-bar ${isInputFocused ? "input-focused" : ""}`}>
      ...
      >
        ...
        <Link to={`/search?q=${inputValue}`}>
          <BtnSearchIcon onClick={handleSearch} />
        </Link>
      </div>
    </div>
  );
